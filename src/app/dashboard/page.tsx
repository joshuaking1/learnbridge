...
export default function DashboardPage() {
    ...
    // --- Render dashboard ---
    return (
        <div className="min-h-screen bg-gradient-to-br from-brand-darkblue to-brand-midblue p-4">
            <div className="container mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <Link href="/dashboard/lesson-planner" className="block">
                        <div className="bg-white/10 backdrop-blur-sm p-6 rounded-lg border border-white/20 hover:bg-white/20 transition-all">
                            <h2 className="text-xl font-arvo font-bold text-white mb-2">Lesson Planner</h2>
                            <p className="text-white/80">Create and manage your lesson plans with AI assistance</p>
                        </div>
                    </Link>

                    <Link href="/dashboard/my-lessons" className="block">
                        <div className="bg-white/10 backdrop-blur-sm p-6 rounded-lg border border-white/20 hover:bg-white/20 transition-all">
                            <h2 className="text-xl font-arvo font-bold text-white mb-2">My Lessons</h2>
                            <p className="text-white/80">View and manage your saved lesson plans</p>
                        </div>
                    </Link>

                    <Link href="/dashboard/assessment-creator" className="block">
                        <div className="bg-white/10 backdrop-blur-sm p-6 rounded-lg border border-white/20 hover:bg-white/20 transition-all">
                            <h2 className="text-xl font-arvo font-bold text-white mb-2">Assessment Creator</h2>
                            <p className="text-white/80">Generate assessments and quizzes for your students</p>
                        </div>
                    </Link>

                    <Link href="/dashboard/my-assessments" className="block">
                        <div className="bg-white/10 backdrop-blur-sm p-6 rounded-lg border border-white/20 hover:bg-white/20 transition-all">
                            <h2 className="text-xl font-arvo font-bold text-white mb-2">My Assessments</h2>
                            <p className="text-white/80">View and manage your saved assessments</p>
                        </div>
                    </Link>

                    <Link href="/dashboard/rubric-generator" className="block">
                        <div className="bg-white/10 backdrop-blur-sm p-6 rounded-lg border border-white/20 hover:bg-white/20 transition-all">
                            <h2 className="text-xl font-arvo font-bold text-white mb-2">Rubric Generator</h2>
                            <p className="text-white/80">Create grading rubrics for your assessments</p>
                        </div>
                    </Link>

                    <Link href="/dashboard/tos-builder" className="block">
                        <div className="bg-white/10 backdrop-blur-sm p-6 rounded-lg border border-white/20 hover:bg-white/20 transition-all">
                            <h2 className="text-xl font-arvo font-bold text-white mb-2">TOS Builder</h2>
                            <p className="text-white/80">Generate terms of service for your educational content</p>
                        </div>
                    </Link>
                </div>
                ...
                {user && user.role === 'admin' && (
                    <div className="mt-8 space-y-6">
                        <div className="p-6 bg-white border rounded-lg shadow space-y-3">
                            <h3 className="text-xl font-semibold text-brand-orange">Admin Tools</h3>
                            <div className="flex flex-wrap gap-2">
                                <Link href="/admin/uploads"><Button variant="secondary">Upload Documents</Button></Link>
                ...
                {user && user.role !== 'teacher' && user.role !== 'student' && user.role !== 'admin' && (
                    <div className="mt-8 p-6 bg-white border rounded-lg shadow">
                        <h3 className="text-xl font-semibold text-brand-orange">Welcome</h3>
...
function StudentContentViewer() {
    ...
    const fetchContent = async () => {
        ...
        try {
            ...
            setErrorLoadingContent(error.message || "Could not load learning material.");
            toast({ title: "Loading Error", description: error.message || "Could not load material.", variant: "destructive" });
        } finally {
    ...
    return (
        <Card>
            ...
            <CardContent className="space-y-4">
                ...
                {/* Fetch Button */}
                <Button onClick={fetchContent} disabled={!selectedSubject || !selectedGrade || isLoadingContent}>
                ...
                <div className="mt-4 space-y-4">
                    ...
                    {!isLoadingContent && !errorLoadingContent && contentChunks.length > 0 && (
                        <ScrollArea className="h-[50vh] p-4 border rounded-md bg-white text-black"> {/* Scrollable content */}
                            <h3 className="text-lg font-semibold mb-3 text-brand-darkblue">Relevant Content:</h3>
